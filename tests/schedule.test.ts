...
import {addDays,generateSchedule,localClock,mondayOf,pairCycle,reminderTypes,assignmentStatus,weeklyTaskPlan} from '../lib/schedule';
...
test('six-week daily cycle gives everyone every weekday once',()=>{
 const a=generateSchedule(testMembers,defaultSettings,'2026-09-29','2026-11-08').filter(a=>a.kind==='daily');
 assert.equal(a.length,36);
 for(const m of testMembers){const own=a.filter(a=>a.member_ids.includes(m.id));assert.equal(own.length,6);assert.equal(new Set(own.map(a=>new Date(a.date+'T12:00:00Z').getUTCDay())).size,6);}
...
test('Monday cycle covers all 15 pairs, five turns each, with balanced blocks',()=>{
 const p=pairCycle(testMembers.map(m=>m.id));assert.equal(p.length,15);assert.equal(new Set(p.map(p=>[...p].sort().join(':'))).size,15);
 for(const m of testMembers)assert.equal(p.flat().filter(id=>id===m.id).length,5);
...
test('a weekly clean is complete only after both members submit',()=>{
 const a=generateSchedule(testMembers,defaultSettings,'2026-10-05','2026-10-05')[0];
...
test('weekly tasks are balanced and rotate away from recently cleaned areas',()=>{
 const tasks=['Kitchen','Oven','Stove','Toilet','Bathroom','Common Space','Lounge room','Laundry'];
 const past={id:'weekly-2026-10-05',date:'2026-10-05',kind:'weekly' as const,member_ids:['a','c'],tasks};
 const current={...past,id:'weekly-2026-10-12',date:'2026-10-12',member_ids:['a','b']};
 const submissions=[{id:'s1',assignment_id:past.id,member_id:'a',tasks:['Kitchen','Oven','Stove','Toilet'],notes:'',status:'approved' as const,submitted_at:'2026-10-05T10:00:00Z',review_note:''}];
 const plan=weeklyTaskPlan(current,[past,current],submissions);
 assert.equal(plan.a.length,4);assert.equal(plan.b.length,4);
 assert.deepEqual([...plan.a,...plan.b].sort(),[...tasks].sort());
 assert.equal(new Set([...plan.a,...plan.b]).size,tasks.length);
 assert.ok(!plan.a.includes('Toilet'));
});
